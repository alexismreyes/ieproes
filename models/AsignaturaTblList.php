 ...
 * Page class
 */
class AsignaturaTblList extends AsignaturaTbl
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AsignaturaTblList";
    ...
    public $FormName = "fasignatura_tbllist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "AsignaturaTblList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'asignatura_tbl';
        $this->TableName = 'asignatura_tbl';
        ...
        // Table object (asignatura_tbl)
        if (!isset($GLOBALS["asignatura_tbl"]) || get_class($GLOBALS["asignatura_tbl"]) == PROJECT_NAMESPACE . "asignatura_tbl") {
            $GLOBALS["asignatura_tbl"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "AsignaturaTblAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "AsignaturaTblDelete";
        $this->MultiUpdateUrl = "AsignaturaTblUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'asignatura_tbl');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "AsignaturaTblView"; // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->id_profesor);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "fasignatura_tblgrid";
        }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fasignatura_tblsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_alumnos_asignatura_tbl");
        $item->CssClass = "text-nowrap";
        ...
        $pages->add("alumnos_asignatura_tbl");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit() && $this->showOptionLink("edit")) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"asignatura_tbl\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasignatura_tbllist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fasignatura_tbllist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'calificacion_tbl') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("calificacion_tbl", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("CalificacionTblList?" . Config("TABLE_SHOW_MASTER") . "=asignatura_tbl&" . GetForeignKeyUrl("fk_id_asignatura", $this->id_asignatura->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("CalificacionTblGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'asignatura_tbl')) {
        ...
        // "detail_alumnos_asignatura_tbl"
        $opt = $this->ListOptions["detail_alumnos_asignatura_tbl"];
        if ($Security->allowList(CurrentProjectID() . 'alumnos_asignatura_tbl') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("alumnos_asignatura_tbl", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("AlumnosAsignaturaTblList?" . Config("TABLE_SHOW_MASTER") . "=asignatura_tbl&" . GetForeignKeyUrl("fk_id_asignatura", $this->id_asignatura->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AlumnosAsignaturaTblGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'asignatura_tbl')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=alumnos_asignatura_tbl");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"asignatura_tbl\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'asignatura_tbl') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                ...
                $item = &$option->add("detailadd_alumnos_asignatura_tbl");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=alumnos_asignatura_tbl");
                $detailPage = Container("AlumnosAsignaturaTblGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'asignatura_tbl') && $Security->canAdd());
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fasignatura_tblsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fasignatura_tblsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fasignatura_tbllist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_asignatura_tbl", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_asignatura_tbl",
    ...
    public function renderRow()
    {
        ...
        // id_asignatura
        $this->id_asignatura->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // id_profesor
            $curVal = strval($this->id_profesor->CurrentValue);
            if ($curVal != "") {
                $this->id_profesor->ViewValue = $this->id_profesor->lookupCacheOption($curVal);
                if ($this->id_profesor->ViewValue === null) { // Lookup from database
                    $filterWrk = SearchFilter("`id_usuario`", "=", $curVal, DATATYPE_NUMBER, "");
                    $lookupFilter = $this->id_profesor->getSelectFilter($this); // PHP
                    $sqlWrk = $this->id_profesor->Lookup->getSql(false, $filterWrk, $lookupFilter, $this, true, true);
                    $conn = Conn();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->id_profesor->Lookup->renderViewRow($rswrk[0]);
                        $this->id_profesor->ViewValue = $this->id_profesor->displayValue($arwrk);
                    } else {
                        $this->id_profesor->ViewValue = FormatNumber($this->id_profesor->CurrentValue, $this->id_profesor->formatPattern());
                    }
                }
            } else {
                $this->id_profesor->ViewValue = null;
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fasignatura_tblsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
