...
class AlumnosAsignaturaTbl extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->fk_id_asignatura->Lookup = new Lookup('fk_id_asignatura', 'asignatura_tbl', false, 'id_asignatura', ["nombre_asignatura","","",""], '', '', [], [], [], [], [], [], '', '', "`nombre_asignatura`");
                break;
            default:
                $this->fk_id_asignatura->Lookup = new Lookup('fk_id_asignatura', 'asignatura_tbl', false, 'id_asignatura', ["nombre_asignatura","","",""], '', '', [], [], [], [], [], [], '', '', "`nombre_asignatura`");
                break;
        ...
        $this->fk_id_alumno = new DbField(
            ...
            -1, // Date/Time format
            false, // Is upload field
            '`EV__fk_id_alumno`', // Virtual expression
            true, // Is virtual
            false, // Force selection
        ...
        $this->fk_id_alumno->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT CONCAT(COALESCE(`nombre_alumno`, ''),'" . ValueSeparator(1, $this->fk_id_alumno) . "',COALESCE(`apellidos_alumno`,''),'" . ValueSeparator(2, $this->fk_id_alumno) . "',COALESCE(`numcarnet_alumno`,'')) FROM `alumnotbl` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id_alumno` = `alumnos_asignatura_tbl`.`fk_id_alumno` LIMIT 1) AS `EV__fk_id_alumno` FROM `alumnos_asignatura_tbl`)";
    ...
    protected function useVirtualFields()
    {
        ...
        if (
            $this->fk_id_alumno->AdvancedSearch->SearchValue != "" ||
            $this->fk_id_alumno->AdvancedSearch->SearchValue2 != "" ||
            ContainsString($where, " " . $this->fk_id_alumno->VirtualExpression . " ")
        ) {
            return true;
        }
        if (ContainsString($orderBy, " " . $this->fk_id_alumno->VirtualExpression . " ")) {
            return true;
        }
        return false;
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->fk_id_alumno->EditValue = $this->fk_id_alumno->CurrentValue;
