 ...
 * Page class
 */
class AsignaturaTblView extends AsignaturaTbl
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AsignaturaTblView";
    ...
    // CSS class/style
    public $CurrentPageName = "AsignaturaTblView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_asignatura") ?? Route("id_asignatura")) !== null) {
            $this->RecKey["id_asignatura"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id_asignatura") ?? Route("id_asignatura")) !== null) {
            ...
            $this->RecKey["id_asignatura"] = $this->id_asignatura->QueryStringValue;
        } elseif (Post("id_asignatura") !== null) {
            $this->id_asignatura->setFormValue(Post("id_asignatura"));
            $this->RecKey["id_asignatura"] = $this->id_asignatura->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_asignatura->setQueryStringValue($keyValue);
            $this->RecKey["id_asignatura"] = $this->id_asignatura->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "AsignaturaTblList"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "AsignaturaTblList"; // No matching record, return to list
                    }
                break;
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("CalificacionTblList?" . Config("TABLE_SHOW_MASTER") . "=asignatura_tbl&" . GetForeignKeyUrl("fk_id_asignatura", $this->id_asignatura->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("CalificacionTblGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'asignatura_tbl')) {
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'asignatura_tbl')) {
        ...
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'asignatura_tbl')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=calificacion_tbl"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
            $detailCopyTblVar .= "calificacion_tbl";
        }
        if ($links != "") {
