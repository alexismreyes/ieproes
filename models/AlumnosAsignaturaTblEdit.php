 ...
 * Page class
 */
class AlumnosAsignaturaTblEdit extends AlumnosAsignaturaTbl
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AlumnosAsignaturaTblEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "AlumnosAsignaturaTblEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_alumnosasignatura") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_alumnosasignatura->setQueryStringValue($keyValue);
                $this->id_alumnosasignatura->setOldValue($this->id_alumnosasignatura->QueryStringValue);
            } elseif (Post("id_alumnosasignatura") !== null) {
                $this->id_alumnosasignatura->setFormValue(Post("id_alumnosasignatura"));
                $this->id_alumnosasignatura->setOldValue($this->id_alumnosasignatura->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_alumnosasignatura") ?? Route("id_alumnosasignatura")) !== null) {
                    $this->id_alumnosasignatura->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->id_alumnosasignatura->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("AlumnosAsignaturaTblList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "AlumnosAsignaturaTblList") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AlumnosAsignaturaTblList") {
                            ...
                            $returnUrl = "AlumnosAsignaturaTblList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_alumnosasignatura->IsDetailKey) {
            $this->id_alumnosasignatura->setFormValue($val);
        }
    }
    ...
    public function renderRow()
    {
        ...
        $this->id_alumnosasignatura->RowCssClass = "row";
    ...
    protected function editRow()
    {
        ...
        // fk_id_alumno
        if ($this->fk_id_alumno->getSessionValue() != "") {
            $this->fk_id_alumno->ReadOnly = true;
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AlumnosAsignaturaTblList"), "", $this->TableVar, true);
        $pageId = "edit";
