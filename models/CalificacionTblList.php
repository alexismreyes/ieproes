...
class CalificacionTblList extends CalificacionTbl
{
    ...
    public function run()
    {
        ...
        if ($this->isGridAdd()) {
            ...
            if ($gridInsert) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("CalificacionTblList");
                    return;
                }
            } else {
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
                if ($this->getCurrentMasterTable() == "alumnotbl") {
                    $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "alumnotbl"); // Add master User ID filter
                }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->observacion_calificacion;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fcalificacion_tbllist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->fk_id_alumno->getSessionValue() != "") {
                ...
                $this->fk_id_alumno->OldValue = $this->fk_id_alumno->CurrentValue;
                $curVal = strval($this->fk_id_alumno->CurrentValue);
                if ($curVal != "") {
                    $this->fk_id_alumno->ViewValue = $this->fk_id_alumno->lookupCacheOption($curVal);
                    if ($this->fk_id_alumno->ViewValue === null) { // Lookup from database
    ...
    protected function editRow()
    {
        ...
        // Check referential integrity for master table 'alumnotbl'
        $detailKeys = [];
        $keyValue = $rsnew['fk_id_alumno'] ?? $rsold['fk_id_alumno'];
        $detailKeys['fk_id_alumno'] = $keyValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcalificacion_tblsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
