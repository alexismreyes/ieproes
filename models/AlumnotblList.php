 ...
 * Page class
 */
class AlumnotblList extends Alumnotbl
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AlumnotblList";
    ...
    public $FormName = "falumnotbllist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "AlumnotblList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "AlumnotblDelete";
        $this->MultiUpdateUrl = "AlumnotblUpdate";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "falumnotblsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->genero_alumno->AdvancedSearch->SearchOperator = @$filter["z_genero_alumno"];
        $this->genero_alumno->AdvancedSearch->SearchCondition = @$filter["v_genero_alumno"];
        $this->genero_alumno->AdvancedSearch->SearchValue2 = @$filter["y_genero_alumno"];
        $this->genero_alumno->AdvancedSearch->SearchOperator2 = @$filter["w_genero_alumno"];
        $this->genero_alumno->AdvancedSearch->save();

        // Field fechanac_alumno
        $this->fechanac_alumno->AdvancedSearch->SearchValue = @$filter["x_fechanac_alumno"];
        $this->fechanac_alumno->AdvancedSearch->SearchOperator = @$filter["z_fechanac_alumno"];
        $this->fechanac_alumno->AdvancedSearch->SearchCondition = @$filter["v_fechanac_alumno"];
        $this->fechanac_alumno->AdvancedSearch->SearchValue2 = @$filter["y_fechanac_alumno"];
        $this->fechanac_alumno->AdvancedSearch->SearchOperator2 = @$filter["w_fechanac_alumno"];
        $this->fechanac_alumno->AdvancedSearch->save();

        // Field direccion_alumno
        $this->direccion_alumno->AdvancedSearch->SearchValue = @$filter["x_direccion_alumno"];
        $this->direccion_alumno->AdvancedSearch->SearchOperator = @$filter["z_direccion_alumno"];
        $this->direccion_alumno->AdvancedSearch->SearchCondition = @$filter["v_direccion_alumno"];
        $this->direccion_alumno->AdvancedSearch->SearchValue2 = @$filter["y_direccion_alumno"];
        $this->direccion_alumno->AdvancedSearch->SearchOperator2 = @$filter["w_direccion_alumno"];
        $this->direccion_alumno->AdvancedSearch->save();

        // Field telefono_alumno
        $this->telefono_alumno->AdvancedSearch->SearchValue = @$filter["x_telefono_alumno"];
        $this->telefono_alumno->AdvancedSearch->SearchOperator = @$filter["z_telefono_alumno"];
        $this->telefono_alumno->AdvancedSearch->SearchCondition = @$filter["v_telefono_alumno"];
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"falumnotbllist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"falumnotbllist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'alumnos_asignatura_tbl')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("alumnos_asignatura_tbl", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("AlumnosAsignaturaTblList?" . Config("TABLE_SHOW_MASTER") . "=alumnotbl&" . GetForeignKeyUrl("fk_id_alumno", $this->id_alumno->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            ...
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'alumnotbl')) {
                ...
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=alumnos_asignatura_tbl");
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"alumnotbl\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"falumnotblsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"falumnotblsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="falumnotbllist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_alumnotbl", "data-rowtype" => ROWTYPE_ADD]);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="falumnotbllist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"falumnotblsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
