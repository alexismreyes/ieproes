...
$Page->showMessage();
?>
<form name="fasignatura_tbladd" id="fasignatura_tbladd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<span id="el_asignatura_tbl_id_profesor">
    <select
        ...
        data-select2-id="fasignatura_tbladd_x_id_profesor"
...
<script>
loadjs.ready("fasignatura_tbladd", function() {
    var options = { name: "x_id_profesor", selectId: "fasignatura_tbladd_x_id_profesor" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasignatura_tbladd.lists.id_profesor?.lookupOptions.length) {
        options.data = { id: "x_id_profesor", form: "fasignatura_tbladd" };
    } else {
        options.ajax = { id: "x_id_profesor", form: "fasignatura_tbladd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fasignatura_tbladd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fasignatura_tbladd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
