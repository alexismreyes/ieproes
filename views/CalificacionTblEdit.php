...
?>
<main class="edit">
<form name="fcalificacion_tbledit" id="fcalificacion_tbledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { calificacion_tbl: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fcalificacion_tbledit;
...
<span id="el_calificacion_tbl_fk_id_asignatura">
    <select
        ...
        data-select2-id="fcalificacion_tbledit_x_fk_id_asignatura"
...
<script>
loadjs.ready("fcalificacion_tbledit", function() {
    var options = { name: "x_fk_id_asignatura", selectId: "fcalificacion_tbledit_x_fk_id_asignatura" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcalificacion_tbledit.lists.fk_id_asignatura?.lookupOptions.length) {
        options.data = { id: "x_fk_id_asignatura", form: "fcalificacion_tbledit" };
    } else {
        options.ajax = { id: "x_fk_id_asignatura", form: "fcalificacion_tbledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_calificacion_tbl_fk_id_alumno">
    <select
        ...
        data-select2-id="fcalificacion_tbledit_x_fk_id_alumno"
        data-table="calificacion_tbl"
...
<script>
loadjs.ready("fcalificacion_tbledit", function() {
    var options = { name: "x_fk_id_alumno", selectId: "fcalificacion_tbledit_x_fk_id_alumno" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcalificacion_tbledit.lists.fk_id_alumno?.lookupOptions.length) {
        options.data = { id: "x_fk_id_alumno", form: "fcalificacion_tbledit" };
    } else {
        options.ajax = { id: "x_fk_id_alumno", form: "fcalificacion_tbledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fcalificacion_tbledit_x_fk_id_evaluacion"
        data-table="calificacion_tbl"
...
<script>
loadjs.ready("fcalificacion_tbledit", function() {
    var options = { name: "x_fk_id_evaluacion", selectId: "fcalificacion_tbledit_x_fk_id_evaluacion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcalificacion_tbledit.lists.fk_id_evaluacion?.lookupOptions.length) {
        options.data = { id: "x_fk_id_evaluacion", form: "fcalificacion_tbledit" };
    } else {
        options.ajax = { id: "x_fk_id_evaluacion", form: "fcalificacion_tbledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="calificacion_tbl" data-field="x_id_calificacion" data-hidden="1" name="x_id_calificacion" id="x_id_calificacion" value="<?= HtmlEncode($Page->id_calificacion->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcalificacion_tbledit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcalificacion_tbledit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
