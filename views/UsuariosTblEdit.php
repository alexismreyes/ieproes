...
?>
<main class="edit">
<form name="fusuarios_tbledit" id="fusuarios_tbledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { usuarios_tbl: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fusuarios_tbledit;
...
<?php if ($Page->id_usuario->Visible) { // id_usuario ?>
    <div id="r_id_usuario"<?= $Page->id_usuario->rowAttributes() ?>>
        <label id="elh_usuarios_tbl_id_usuario" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_usuario->caption() ?><?= $Page->id_usuario->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="usuarios_tbl" data-field="x_id_usuario" data-hidden="1" name="x_id_usuario" id="x_id_usuario" value="<?= HtmlEncode($Page->id_usuario->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_usuarios_tbl_tipo_usuario">
    <select
        ...
        data-select2-id="fusuarios_tbledit_x_tipo_usuario"
...
<script>
loadjs.ready("fusuarios_tbledit", function() {
    var options = { name: "x_tipo_usuario", selectId: "fusuarios_tbledit_x_tipo_usuario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusuarios_tbledit.lists.tipo_usuario?.lookupOptions.length) {
        options.data = { id: "x_tipo_usuario", form: "fusuarios_tbledit" };
    } else {
        options.ajax = { id: "x_tipo_usuario", form: "fusuarios_tbledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group">
    <input type="password" name="x_password_usuario" id="x_password_usuario" autocomplete="new-password" data-field="x_password_usuario" value="<?= $Page->password_usuario->EditValue ?>" size="30" maxlength="30" placeholder="<?= HtmlEncode($Page->password_usuario->getPlaceHolder()) ?>"<?= $Page->password_usuario->editAttributes() ?> aria-describedby="x_password_usuario_help">
...
<?php if (!$Security->isAdmin() && $Security->isLoggedIn()) { // Non system admin ?>
<?php if (SameString($Page->id_usuario->CurrentValue, CurrentUserID())) { ?>
    <span<?= $Page->parent_id_usuario->viewAttributes() ?>>
    <input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->parent_id_usuario->getDisplayValue($Page->parent_id_usuario->EditValue))) ?>"></span>
    <input type="hidden" data-table="usuarios_tbl" data-field="x_parent_id_usuario" data-hidden="1" name="x_parent_id_usuario" id="x_parent_id_usuario" value="<?= HtmlEncode($Page->parent_id_usuario->CurrentValue) ?>">
<?php } else { ?>
<span id="el_usuarios_tbl_parent_id_usuario">
    <select
        ...
        data-select2-id="fusuarios_tbledit_x_parent_id_usuario"
        data-table="usuarios_tbl"
...
<script>
loadjs.ready("fusuarios_tbledit", function() {
    var options = { name: "x_parent_id_usuario", selectId: "fusuarios_tbledit_x_parent_id_usuario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusuarios_tbledit.lists.parent_id_usuario?.lookupOptions.length) {
        options.data = { id: "x_parent_id_usuario", form: "fusuarios_tbledit" };
    } else {
        options.ajax = { id: "x_parent_id_usuario", form: "fusuarios_tbledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fusuarios_tbledit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fusuarios_tbledit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
