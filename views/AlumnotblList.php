...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="falumnotblsrch" id="falumnotblsrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="on">
<div id="falumnotblsrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { alumnotbl: currentTable } });
var currentForm;
var falumnotblsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="falumnotblsrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="falumnotblsrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="falumnotblsrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="falumnotblsrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_alumnotbl" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_alumnotbllist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Page->fechanac_alumno->Visible) { // fechanac_alumno ?>
        <th data-name="fechanac_alumno" class="<?= $Page->fechanac_alumno->headerCellClass() ?>"><div id="elh_alumnotbl_fechanac_alumno" class="alumnotbl_fechanac_alumno"><?= $Page->renderFieldHeader($Page->fechanac_alumno) ?></div></th>
<?php } ?>
<?php if ($Page->direccion_alumno->Visible) { // direccion_alumno ?>
        <th data-name="direccion_alumno" class="<?= $Page->direccion_alumno->headerCellClass() ?>"><div id="elh_alumnotbl_direccion_alumno" class="alumnotbl_direccion_alumno"><?= $Page->renderFieldHeader($Page->direccion_alumno) ?></div></th>
<?php } ?>
