...
?>
<main class="edit">
<form name="fprofesor_tbledit" id="fprofesor_tbledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { profesor_tbl: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fprofesor_tbledit;
...
<?php if ($Page->id_profesor->Visible) { // id_profesor ?>
    <div id="r_id_profesor"<?= $Page->id_profesor->rowAttributes() ?>>
        <label id="elh_profesor_tbl_id_profesor" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_profesor->caption() ?><?= $Page->id_profesor->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_profesor->getDisplayValue($Page->id_profesor->EditValue))) ?>"></span>
<input type="hidden" data-table="profesor_tbl" data-field="x_id_profesor" data-hidden="1" name="x_id_profesor" id="x_id_profesor" value="<?= HtmlEncode($Page->id_profesor->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fprofesor_tbledit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fprofesor_tbledit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
